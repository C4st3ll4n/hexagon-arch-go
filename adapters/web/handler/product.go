...
func MakeProductHandler(r *mux.Router, n negroni.Negroni, service application.ProductServiceInterface) {
	...
	r.Handle("/product/{id}/enable", n.With(
		negroni.Wrap(enableProduct(service)))).Methods("PUT", "OPTIONS")

	r.Handle("/product/{id}/disable", n.With(
		negroni.Wrap(disableProduct(service)))).Methods("PUT", "OPTIONS")
}
...
func enableProduct(service application.ProductServiceInterface) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		err = json.NewEncoder(w).Encode(enabledProduct)
		if err != nil {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
...
func disableProduct(service application.ProductServiceInterface) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		err = json.NewEncoder(w).Encode(disabledProduct)
		if err != nil {
			w.WriteHeader(http.StatusInternalServerError)
			w.Write(jsonError(err.Error()))
